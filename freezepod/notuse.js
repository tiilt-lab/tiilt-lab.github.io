...
function connect() {
    ...
    if (!navigator.bluetooth) {
        ...
        .then(service => {
            addLog("<font color='green'>OK</font>", true);
            addLog("Getting Button matrix state characteristic... ", false);
            addLog()
...
function connectSecond() {
    ...
    if (!navigator.bluetooth) {
        ...
        .then(service => {
            ...
            .then(matrixChar => {
                addLog("<font color='green'>OK</font>", true);
                ledMatrixCharacteristicSecond = matrixChar;
                
                addLog("Getting LED text characteristic... ", false);
                service.getCharacteristic(microbitUuid.ledText[0])
                .then(textChar => {
                    addLog("<font color='green'>OK</font>", true);
                    ledTextCharacteristicSecond = textChar;
                    addLog("Getting scrolling delay characteristic... ", false);
                    service.getCharacteristic(microbitUuid.scrollingDelay[0])
                    .then(delayChar => {
                        scrollingDelayCharacteristicSecond = delayChar;
                        addLog("<font color='green'>OK</font>", true);
                        service.getCharacteristic(microbitUuid.buttonService[0])
...
function disconnect() {
    ...
    console.log(secondBluetoothDevice)
  ...
  if (!secondBluetoothDevice) {
        addLogError("There is no device connected.");
    } else {
        if (secondBluetoothDevice.gatt.connected) {
            secondBluetoothDevice.gatt.disconnect();
            if (!secondBluetoothDevice.gatt.connected) {
                addLog("<font color='green'>OK</font>", true);
            };
        } else {
...
/*
function readLedMatrixBoth() {
    addLog("Reading LED matrix... ", false);
    if (!bluetoothDevice || !secondBluetoothDevice) {
        addLogError("There are not two devices.");
    } else {
        if (secondBluetoothDevice.gatt.connected && secondBluetoothDevice.gatt.connected) {
            if (!ledMatrixCharacteristic || !ledMatrixCharacteristicSecond) {
                addLogError("There is no LED Matrix characteristics.");
            } else {
                ledMatrixCharacteristic.readValue()
                .then(value => {
                    for (let rowIndex = 0; rowIndex < 5; rowIndex++) {
                ...
                ledMatrixCharacteristicSecond.readValue()
                .then(value => {
                    for (let rowIndex = 0; rowIndex < 5; rowIndex++) {
                        let row = value.getUint8(rowIndex);
                        for (let columnIndex = 0; columnIndex < 5; columnIndex++) {
            ...
            addLogError("There is no devices connected.");
        };
    };
}
...
function writeLedMatrixSecond() {
    addLog("Writing LED matrix... ", false);
    if (!secondBluetoothDevice) {
        addLogError("There is no device connected.");
    } else {
        if (secondBluetoothDevice.gatt.connected) {
            if (!ledMatrixCharacteristicSecond) {
                addLogError("There is no LED Matrix characteristic.");
            } else {
                let buffer = new ArrayBuffer(5);
                let ledMatrix = new DataView(buffer);
                ...
                ledMatrixCharacteristicSecond.writeValue(ledMatrix)
                .then(_ => {
                    addLog("<font color='green'>OK</font>", true);
                })
                .catch(error => {
...
function writeLedMatrixBoth() {
    addLog("Writing LED matrix... ", false);
    if (!bluetoothDevice || !secondBluetoothDevice) {
        addLogError("There is not two devices connected.");
    } else {
        if (bluetoothDevice.gatt.connected && secondBluetoothDevice.gatt.connected) {
            if (!ledMatrixCharacteristic || !ledMatrixCharacteristicSecond ) {
                addLogError("There is no LED Matrix characteristics.");
            } else {
                let buffer = new ArrayBuffer(5);
        ...
        if (secondBluetoothDevice.gatt.connected) {
            if (!ledTextCharacteristic) {
                ...
                if (!("TextEncoder" in window)) {
                    ...
                    let enc = new TextEncoder();
                    ledTextCharacteristicSecond.writeValue(enc.encode(document.getElementById("ledTextText").value))
                    .then(_ => {
                        addLog("<font color='green'>OK</font>", true);
                    })
                    .catch(error => {
...
function writeLedTextBoth() {
    addLog("Writing LED text... ", false);
    if (!bluetoothDevice || !secondBluetoothDevice) {
        addLogError("There is no devices connected.");
    } else {
        if (bluetoothDevice.gatt.connected && secondBluetoothDevice.gatt.connected) {
            if (!ledTextCharacteristic || !ledTextCharacteristicSecond) {
                addLogError("There is no LED Text characteristics.");
            } else {
                if (!("TextEncoder" in window)) {
                    addLogError("Sorry, this browser does not support TextEncoder.");
                } else {
...
function readScrollingDelay() {
    ...
    if (!bluetoothDevice || !secondBluetoothDevice) {
        ...
        if (bluetoothDevice.gatt.connected && secondBluetoothDevice.gatt.connected) {
            if (!scrollingDelayCharacteristic || !scrollingDelayCharacteristicSecond) {
                addLogError("There is no Scolling Delay characteristics.");
            } else {
                ...
                scrollingDelayCharacteristicSecond.readValue()
                .then(value => {
                    document.getElementById("readScrollingDelayText").value = value.getUint16(0, true); // Little Endian
                    addLog("<font color='green'>OK</font>", true);
                })
    ...
    if (!bluetoothDevice || !secondBluetoothDevice) {
        ...
        if (bluetoothDevice.gatt.connected && secondBluetoothDevice.gatt.connected) {
            if (!scrollingDelayCharacteristic || !scrollingDelayCharacteristicSecond) {
                ...
                scrollingDelayCharacteristicSecond.writeValue(scrollingDelay)
                .then(_ => {
                    addLog("<font color='green'>OK</font>", true);
                })
                .catch(error => {
...
function startGame(){
  ...
  let arrDevices = [bluetoothDevice, secondBluetoothDevice]
  let arrChar = [ledMatrixCharacteristic, ledMatrixCharacteristicSecond]
  
  /*
  let dic = {"bluetoothDevice": ledMatrixCharacteristic, "secondBluetoothDevice": ledMatrixCharacteristicSecond};
  console.log([(bluetoothDevice).toString()])
   console.log(dic)
   */
  
  let num = randomIntFromInterval(0, arrDevices.length-1)
  let whichMicrobit = arrDevices[num]
  let led
  addLog(whichMicrobit.name)
  console.log(whichMicrobit)
  if (whichMicrobit==bluetoothDevice){
    led = ledMatrixCharacteristic

  }
  else if (whichMicrobit==secondBluetoothDevice){
    led = ledMatrixCharacteristicSecond
  }
  let buffer = new ArrayBuffer(5);
                let ledMatrix = new DataView(buffer);
                for (let rowIndex = 0; rowIndex < 5; rowIndex++) {
                ...
                arrChar[num].writeValue(ledMatrix)
                .then(_ => {
                    addLog("<font color='green'>OK</font>", true);
                })
                .catch(error => {
  ...
  for (let i =0; i<arrDevices.length; i++){
    console.log(whichMicrobit)
    console.log(arrDevices[i])
    
    if (arrDevices[i] != whichMicrobit){
      console.log("HERE")
      console.log(arrDevices[i])
      let buffer2 = new ArrayBuffer(5);
                    let ledMatrix2 = new DataView(buffer);
                    for (let rowIndex = 0; rowIndex < 5; rowIndex++) {
                        /*ledMatrix.setUint8(rowIndex, 0);*/
                        for (let columnIndex = 0; columnIndex < 5; columnIndex++) {
                            console.log((document.getElementById((rowIndex+1).toString() + (5-columnIndex).toString())))
                            ledMatrix2.setUint8(rowIndex, ledMatrix2.getUint8(rowIndex) << (document.getElementById((rowIndex+1).toString() + (5-columnIndex).toString()).checked| columnIndex));
                        };
                    };
                    arrChar[i].writeValue(ledMatrix2)
                    .then(_ => {
                        addLog("<font color='green'>OK</font>", true);
                    })
                    .catch(error => {
